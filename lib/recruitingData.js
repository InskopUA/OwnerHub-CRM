export const statuses = [
  ["new", "New Lead"],
  ["contact_attempted", "Contacting"],
  ["qualified", "Qualified"],
  ["docs_requested", "Documents"],
  ["quote_pending", "Insurance"],
  ["agreement_sent", "Agreement"],
  ["safety_onboarding", "Onboarding"],
...
const primaryStatusMap = Object.fromEntries(statuses);

export const normalizeCandidateStatus = (status) =>
  Object.prototype.hasOwnProperty.call(legacyStatusGroups, status) ? legacyStatusGroups[status] : status;

export const statusMap = {
  ...Object.fromEntries(Object.entries(legacyStatusGroups).map(([legacy, group]) => [legacy, primaryStatusMap[group]])),
  ...primaryStatusMap
};
...
export const callSteps = {
  ...
  finish_followup: { finish: true, outcome: "followup", say: { ru: "Звонок завершён. Follow-up сохранён, кандидат останется на этапе Contacting.", en: "Call completed. The follow-up is saved and the candidate remains at Contacting." } },
  ...
  finish_lost: { finish: true, outcome: "lost", say: { ru: "Кандидат отмечен как Lost. Причина сохранена в карточке.", en: "Candidate marked as Lost. The reason is saved in the profile." } },
